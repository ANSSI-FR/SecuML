...
class GroundTruth(object):
    ...
    def _check(self, already_loaded):
        ...
        if not path.isfile(self.filepath):
            ...
            return
        # Check the hash
        self.ground_truth_hash = compute_hash(self.filepath)
    ...
    def _add_to_db(self):
        if self.secuml_conf.db_type == 'mysql':
            mysql_specific.load_ground_truth(self.cursor, self.filepath,
                                             self.dataset_conf.dataset_id)
        elif self.secuml_conf.db_type == 'postgresql':
            postgresql_specific.load_ground_truth(self.cursor, self.filepath,
                                                  self.dataset_conf.dataset_id)
        else:
            assert(False)
