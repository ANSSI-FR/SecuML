...
function callback() {
    ...
    d3.json(buildQuery('getAnnotationsTypes', [exp_id, iteration]),
            function(data) {
...
function displayInstancesToAnnotate(label, type) {
    return function () {
        ...
        var clustering_exp = annotations_types[label];
...
function displayNavbars(type, annotations_type, clustering_exp) {
    ...
    var panel_body = createPanel('panel-' + type, 'row', 'Annotation Queries',
                                 nav_bars);
