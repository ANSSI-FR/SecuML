...
class Idents(object):
    ...
    def _check(self, already_loaded):
        ...
        # Check the hash
        self.idents_hash = dir_tools.compute_hash(self.filepath)
    ...
    def _add_to_db(self):
        if self.secuml_conf.db_type == 'mysql':
            mysql_specific.loadIdents(self.cursor, self.filepath,
                                      self.dataset_conf.dataset_id)
        elif self.secuml_conf.db_type == 'postgresql':
            postgresql_specific.loadIdents(self.cursor, self.filepath,
                                           self.dataset_conf.dataset_id)
        else:
            assert(False)
