...
function setInstancesSettings(train_test, experiment_id) {
    if (train_test == 'validation') {
        var test_conf = conf.test_exp_conf;
        inst_exp_id   = test_conf.experiment_id;
        inst_annotations_id = 'None'; // ground truth cannot be changed.
        inst_annotations_type = 'ground_truth';
        inst_dataset_id = test_conf.dataset_conf.dataset_id;
    } else if (train_test == 'train' || train_test == 'cv') {
        inst_exp_id = experiment_id;
        inst_annotations_id = conf.annotations_conf.annotations_id;
        inst_annotations_type = conf.annotations_conf.annotations_type;
        inst_dataset_id = conf.dataset_conf.dataset_id;
    } else if (train_test == 'test') {
        var test_conf = conf.core_conf.test_conf;
        if (test_conf.method == 'dataset') {
            var test_conf = conf.test_exp_conf;
            inst_exp_id = test_conf.experiment_id;
            inst_annotations_id = test_conf.annotations_conf.annotations_id;
            inst_annotations_type = test_conf.annotations_conf.annotations_type;
            inst_dataset_id = test_conf.dataset_conf.dataset_id;
        } else {
            inst_exp_id = experiment_id;
            inst_annotations_id = conf.annotations_conf.annotations_id;
...
function qPrintWeightedFeatures(conf) {
    if (conf.__type__.split('Conf')[0] == 'Classification') {
        if (conf.core_conf.classifier_conf.feature_importance == 'weight') {
            return true;
        }
    }
    return false;
...
function printInstanceInformation(selected_id, proba=null, suggested_label=null,
                                  suggested_family=null) {
    printInstanceIndent(selected_id, proba);
...
function printInstanceIndent(selected_id, proba) {
    d3.json(buildQuery('getIdent', [inst_exp_id, selected_id]),
        function(error, data) {
...
function updateInstanceLabel(selected_id) {
    var query = buildQuery('getAnnotation', [inst_annotations_type,
                                             inst_annotations_id,
                                             inst_dataset_id, selected_id]);
...
function displayFamilySelector(label_row, label) {
    ...
    // Family values
    var query = buildQuery('getLabelsFamilies', [inst_annotations_type,
                                                 inst_annotations_id,
                                                 inst_dataset_id,
                                                 'None']);
    jQuery.getJSON(query, function(data) {
...
function displayUpdateAnnotationForm(form) {
    ...
    button.addEventListener('click', removeAnnotationCallback(experiment_id,
                                                          inst_annotations_id));
...
function addAnnotationCallback(experiment_id, inst_annotations_id,
        label_iteration) {
    return function() {
        ...
        if (instance_id) {
            ...
            var query = buildQuery('updateAnnotation', [experiment_id,
                                                        inst_annotations_id,
                                                        label_iteration,
                                                        instance_id,
                                                        label, family,
...
function removeAnnotationCallback(experiment_id, inst_annotations_id) {
    return function() {
        var instance_id = getCurrentInstance();
        var query = buildQuery('removeAnnotation', [experiment_id,
                                                    inst_annotations_id,
                                                    instance_id]);
        $.ajax({url: query,
            success: function(data) {
