...
loadConfigurationFile(experiment_id);

function loadConfigurationFile(experiment_id) {
...
function load_features_info() {
    var query = buildQuery('getFeaturesInfo', [experiment_id]);
...
function display_sorted_features() {
    ...
    var query = buildQuery('getSortedFeatures', [experiment_id,
                                                 selected_criterion]);
    $.getJSON(query, function(data) {
...
function display_criterion_density(criterion) {
    ...
    var path = buildQuery('getCriterionDensity', [experiment_id, criterion]);
...
function display_boxplot(feature) {
    ...
    var path = buildQuery('getStatsPlot', [experiment_id, 'boxplot', feature]);
    picture.src = path;
}

function display_histogram(feature) {
    var query = buildQuery('getStatsPlot', [experiment_id, 'histogram',
                                            feature]);
    var hist = $('#hist')[0];
    $.getJSON(query, function (data) {
...
function display_binary_histogram(feature) {
    var query = buildQuery('getStatsPlot', [experiment_id, 'binary_histogram',
                                            feature]);
    var hist = $('#hist')[0];
    $.getJSON(query, function (data) {
        var options = barPlotOptions(data);
...
function display_density(feature) {
    ...
    var path = buildQuery('getStatsPlot', [experiment_id, 'density', feature]);
    picture.src = path;
}

function display_scores(feature) {
    var body = document.getElementById('scores_tab');
    var query = buildQuery('getFeatureScores', [experiment_id, feature]);
    $.getJSON(query, function (data) {
