...
function loadConfigurationFile() {
    $.getJSON(buildQuery('getConf', [experiment_id]),
            function(data) {
                ...
                updateInstancesDisplay(experiment_id, selected_kind);
            });
}

function updateInstancesDisplay(experiment_id, label) {
    if (!label) {
        label = 'all';
    }
    var query = buildQuery('supervisedLearningMonitoring',
            [conf.experiment_id, train_test, 'errors', fold_id]);
    $.getJSON(query,
            function(data) {
                if (label == 'all') {
...
function displayNavigationPanel(selected_kind) {
    ...
    function radio_callback() {
        updateInstancesDisplay(experiment_id,
