function displayPerformanceTabs(conf, train_test) {
    ...
    displayCurvesTabs(conf, train_test);
}

function displayCurvesTabs(conf, train_test) {
...
function updatePerformanceDisplay(conf, train_test, sup_exp, selected_fold) {
    ...
    if (! classifier_conf.families_supervision) {
      ...
      displayPrecisionRecall(precision_recall, conf, train_test, exp,
                             selected_fold)
      // Confusion Matrix
...
function displayPerfIndicators(div_obj, conf, train_test, exp, selected_fold) {
  if (classifier_conf.probabilist_model && !classifier_conf.families_supervision) {
              ...
              function(data) {
                ...
                if (selected_fold != 'all') {
                  ...
                  var errors_query = buildQuery('errors',
                          [exp, train_test, selected_fold, 'FN']);
                ...
                if (selected_fold != 'all') {
                  ...
                  var errors_query = buildQuery('errors',
                          [exp, train_test, selected_fold, 'FP']);
...
function displayPrecisionRecall(div, conf, train_test, exp, selected_fold) {
  var precision_recall_path = buildQuery('supervisedLearningMonitoring',
                                         [exp,
                                          train_test,
                                          'false_discovery_recall_curve',
                                          selected_fold]);
  var picture = document.createElement('img');
  picture.setAttribute('class', 'img-responsive');
