...
function createTrainTestMonitoring(train_test, col_size, parent_div) {
    ...
    var monitoring = createPanel('panel-primary', col_size, panel_title,
                                 parent_div);
...
function displayAlertsButton(buttons_group, button_label, buttons_title,
    ...
    label_button.addEventListener('click', function() {
        if (button_label != 'clustering') {
            ...
            clustering_exp_id = getAlertsClusteringExperimentId(experiment_id)
...
function displayAlertsButtons(selected_fold) {
    ...
    var alerts_div = createPanel('panel-danger', null, 'Alerts Analysis',
                                 test_monitoring);
...
function displayCoefficients(conf, sup_exp, train_test, selected_fold) {
        ...
        var query = buildQuery('getTopModelFeatures', [exp, num_coeff_model,
...
function checkDisplayCoefficients(train_test, exp_type, conf) {
    if (train_test == 'train' || train_test == 'cv') {
        if (exp_type == 'Classification' || exp_type == 'ActiveLearning') {
            if (!classifier_conf.families_supervision) {
                if (classifier_conf.feature_importance) {
                    return true;
                }
            }
        }
...
function checkDisplayPerformance(train_test, conf, exp_type) {
    ...
    if (exp_type == 'Classification') {
        var test_method = conf.core_conf.test_conf.method;
        if (test_method == 'unlabeled') {
            return false;
        }
        if (test_method == 'dataset') {
            return conf.validation_has_ground_truth;
        }
        return true;
...
function checkDisplayAlerts(conf) {
  console.log(conf);
  if (conf.core_conf.test_conf.alerts_conf) {
      return !classifier_conf.families_supervision;
  }
  return false;
}
