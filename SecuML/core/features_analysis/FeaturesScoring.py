...
class FeaturesScoring(object):
    ...
    def __init__(self, instances):
        ...
        self.annotated_instances = instances.getAnnotatedInstances()
    ...
    def _set_scoring_func(self):
        ...
        if self.annotated_instances.numInstances() > 0:
    ...
    def compute_scoring_func(self, func):
        if func == 'variance':
            ...
            annotations = self.instances.annotations.getLabels()
            return features.var(axis=0), None
        features = self.annotated_instances.features.getValues()
        annotations = self.annotated_instances.annotations.getLabels()
        if func == 'f_classif':
