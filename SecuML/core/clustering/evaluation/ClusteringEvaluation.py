...
class ClusteringEvaluation(object):
    ...
    def generateEvaluation(self, output_dir, quick=False):
        ...
        self.silhouette.generateEvaluation(output_dir, self.assigned_clusters,
                                           quick=quick)
        if self.instances.hasGroundTruth():
            gt_labels = self.instances.ground_truth.getLabels()
            gt_families = self.instances.ground_truth.getFamilies()
