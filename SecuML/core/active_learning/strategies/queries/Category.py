...
class Category(object):
    ...
    def generateQuery(self, instance_id, predicted_proba,
                                ...
                                confidence=None):
        return Query(instance_id, predicted_proba,
                               suggested_label, suggested_family,
    ...
    def to_json(self):
        ...
        for kind, queries in self.annotation_queries.items():
            ...
            for q, query in enumerate(queries):
                obj['annotation_queries'][kind].append(query.to_json())
        return obj
    ...
    def generateQueries(self, cluster_strategy):
    ...
    def queryHighLikelihoodInstances(self, drop_instances, num_instances):
        if num_instances == 0:
            return []
        queries_df = self.getSelectedInstancesDataframe(drop_instances)
        matrix_tools.sort_data_frame(queries_df, 'likelihood', False, True)
        queries_df = queries_df.head(num_instances)
    ...
    def addAnnotationQueries(self, kind, confidence, queries_df):
        ...
        for index, row in queries_df.iterrows():
            query = self.generateQuery(int(index), row['likelihood'],
