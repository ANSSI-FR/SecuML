...
class AladinQueries(Queries):
    ...
    def createNaiveBayesConf(self):
        test_conf = UnlabeledLabeledConf(self.conf.logger, None)
    ...
    def runNaiveBayes(self):
        ...
        all_datasets.setDatasets(train_instances, None, None)
        ...
        for i, family in enumerate(list(self.lr_class_labels)):
            ...
            if selection.shape[0] > 0:
                family_scores = self.scores.loc[self.scores[classifier +
                                                            '_prediction'] == family]
                family_scores = matrix_tools.sort_data_frame(family_scores,
                                             classifier + '_score', True, False)
            else:
