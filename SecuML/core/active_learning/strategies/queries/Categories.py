...
from SecuML.core.classification.conf.test_conf.UnlabeledLabeledConf import UnlabeledLabeledConf
...
class Categories(object):
    ...
    def to_json(self):
        obj = {}
        for c in range(self.num_categories):
            obj[str(c)] = self.categories[c].to_json()
        return obj
    ...
    def generateQueries(self, conf):
        self.setCategoryNumAnnotations(conf)
        for c, category in enumerate(self.categories):
            category.generateQueries(conf.cluster_strategy)
    ...
    def computeCategoryStabilityWeights(self):
        ...
        previous_categories = getattr(
            self.iteration.prev_iter.annotations, self.label).categories
    ...
    def computeCategoryHomogeneityWeights(self):
        weights = [1] * self.num_categories
        current_categories = self.categories
        if self.iteration.prev_iter is not None:
            previous_categories = getattr(
                self.iteration.prev_iter.annotations, self.label).categories
            previous_annotation_queries = getattr(
                self.iteration.prev_iter.annotations, self.label).annotation_queries
            if previous_categories is not None:
                current_categories_labels = [current_categories.getClusterLabel(
                    c) for c in range(current_categories.num_categories)]
                previous_categories_labels = [previous_categories.getClusterLabel(
    ...
    def trainNaiveBayes(self):
        ...
        datasets.setDatasets(self.instances, None, None)
    ...
    def getNaiveBayesConf(self):
        test_conf = UnlabeledLabeledConf(self.iteration.conf.logger, None)
        return GaussianNaiveBayesConf(4, False, True, test_conf,
                                      self.iteration.conf.logger)
