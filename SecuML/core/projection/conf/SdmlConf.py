...
from SecuML.core.projection.algos.Sdml import Sdml
...
class SdmlConf(SemiSupervisedProjectionConf):

    def __init__(self, logger, families_supervision=None):
        SemiSupervisedProjectionConf.__init__(self, logger, Sdml,
                                    families_supervision=families_supervision)

    @staticmethod
    def from_json(obj, logger):
        conf = SdmlConf(logger,
    ...
    def fromArgs(args, logger):
        return SdmlConf(logger, families_supervision=args.families_supervision)


ProjectionConfFactory.getFactory().registerClass('SdmlConf', SdmlConf)
