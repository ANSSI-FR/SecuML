...
class Category(object):
    ...
    def toJson(self):
        ...
        for kind, queries in self.annotation_queries.iteritems():
    ...
    def generateAnnotationQueries(self, cluster_strategy):
        ...
        for q, queries_type in enumerate(queries_types):
            ...
            if queries_type == 'center':
                queries = self.queryHighLikelihoodInstances(annotated_instances, num_queries)
            elif queries_type == 'anomalous':
                queries = self.queryLowLikelihoodInstances(annotated_instances, num_queries)
            elif queries_type == 'uncertain':
                queries = self.queryUncertainInstances(annotated_instances, num_queries)
            elif queries_type == 'random':
                queries = self.queryRandomInstances(annotated_instances, num_queries)
            else:
                raise ValueError()
    ...
    def getSelectedInstancesDataframe(self, drop_instances):
        if drop_instances is None:
            ...
            selected_instances = [x for x in self.instances_ids if x not in drop_instances]
        selected_df = self.df.loc[map(str, selected_instances), :]
        return selected_df
