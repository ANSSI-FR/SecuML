...
from SecuML.Projection.Algorithms.Sdml import Sdml
...
class SdmlConfiguration(SemiSupervisedProjectionConfiguration):
    ...
    def __init__(self, families_supervision = None):
        SemiSupervisedProjectionConfiguration.__init__(self, Sdml, families_supervision = families_supervision)

    @staticmethod
    def fromJson(obj):
        conf = SdmlConfiguration(families_supervision = obj['families_supervision'])
...
ProjectionConfFactory.getFactory().registerClass('SdmlConfiguration',
        SdmlConfiguration)
