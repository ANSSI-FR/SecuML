...
class FamiliesMonitoring(object):
    ...
    def addFold(self, predicted_proba, testing_labels, testing_sublabels):
        self.num_instances += len(predicted_proba)
        if self.num_instances > 150000:
            return
        self.all_predictions += list(predicted_proba)
        for i in range(len(predicted_proba)):
            sublabel = testing_sublabels[i]
            proba = predicted_proba[i]
            if testing_labels[i]:
                if sublabel not in self.malicious_families:
                    self.malicious_families[sublabel] = []
                self.malicious_families[sublabel].append(proba)
            else:
                if sublabel not in self.benign_families:
                    self.benign_families[sublabel] = []
                self.benign_families[sublabel].append(proba)
    ...
    def computeFpFn(self):
        ...
        for family in self.malicious_families.keys():
            ...
            self.detection_rates.loc[:, family]  = perf
        self.detection_rates.loc[:, 'Mean'] = self.detection_rates.loc[:, self.malicious_families.keys()].mean(axis = 1)
        ## False Alarm Rates
        ...
        for family in self.benign_families.keys():
            ...
            perf = 1 - perf / num_predictions
            perf = interp(thresholds, f_thresholds, perf)
            self.false_alarm_rates.loc[:, family]  = perf
        self.false_alarm_rates.loc[:, 'Mean'] = self.false_alarm_rates.loc[:, self.benign_families.keys()].mean(axis = 1)
    ...
    def display(self, directory):
        if self.num_instances < 150000:
            ...
            self.displayLabelsDistribution(directory)
    ...
    def displayFpFnThresholds(self, directory):
        ...
        ax.plot(self.false_alarm_rates.loc[:, 'Mean'], 
                self.detection_rates.loc[:, 'Mean'])
        ax.set_xlabel('False Alarm Rate')
        ax.set_ylabel('Detection Rate')
        ax.set_xlim([-0.05, 1.05])
        ax.set_ylim([-0.05, 1.05])
        filename = directory + 'fp_fn_families_thresholds.png'
    ...
    def displayFamiliesDistribution(self, directory, label = None):
        ...
        for family in families:
            ...
            plt.close(fig_f)
            i += 1
        lgd = ax.legend(bbox_to_anchor = (0., 0.95, 1., .102), loc = 3,
    ...
    def displayLabelsDistribution(self, directory):
        proba = {}
        proba['malicious'] = []
        proba['benign'] = []
        for family in self.malicious_families:
            proba['malicious'] += self.malicious_families[family]
        for family in self.benign_families:
            proba['benign'] += self.benign_families[family]
        bandwidth = 0.1
        num_points = 50
        eps = 0.00001
        kde = KernelDensity(kernel = 'gaussian', bandwidth = bandwidth)
        ...
        filename  = directory
        filename += 'labels_distributions.png'
        fig.savefig(filename)
        plt.close(fig)
