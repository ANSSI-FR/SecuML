...
def addLabel(experiment_id, inst_experiment_label_id, iteration_number, instance_id,
    ...
    dataset_id = db_tables.getDatasetFromExperiment(session, inst_experiment_label_id)
...
def getFamiliesInstances(experiment_id, label, iteration_max):
    ...
    if iteration_max == 'None':
        ...
        iteration_max = int(iteration_max)
    families = labels_tools.getLabelsFamilies(session, experiment_label_id, iteration_max = iteration_max)[label]
    families_instances = {}
    for f in families:
        families_instances[f] = labels_tools.getLabelFamilyIds(session, experiment_label_id, label, family = f,
...
@app.route('/changeFamilyLabel/<experiment_id>/<experiment_label_id>/<label>/<family>/')
def changeFamilyLabel(experiment_id, experiment_label_id, label, family):
    experiment = updateCurrentExperiment(experiment_id)
    experiment_label_id = experiment.oldest_parent
