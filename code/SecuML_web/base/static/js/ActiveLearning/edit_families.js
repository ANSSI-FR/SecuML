...
function displayFamilySelector(label_row, label, multiple, families_values) {
  ...
  var selector_size = 6;
  createSelectList(label + '_family_selector', selector_size, null, col, label_title, multiple);
...
function displayFamiliesSelectors(panel, multiple = false) {
  ...
  jQuery.getJSON(query, function(data) {
        ...
        var benign_col = displayFamilySelector(fieldset, 'benign', multiple, data);
  });
...
function displayChange(modif_div) {
  ...
  cleanDiv(modif_div.id);
  var panel = createPanel('panel-primary', 'row', 'Swap Malicious/Benign for a Family', modif_div);
  displayFamiliesSelectors(panel);
  // Button
  var input_group = createDivWithClass(null, 'input-group', panel);
  var button_span = document.createElement('span');
...
function displayMerge(modif_div) {
  ...
  cleanDiv(modif_div.id);
  var panel = createPanel('panel-primary', 'row', 'Merge Several Families', modif_div);
...
function displayAvailableActions(edit_panel) {
  var select_actions_col = createPanel('panel-primary', 'col-md-3', 'Select an Action', edit_panel);
  ...
  var modif_div = createDivWithClass('modif_div', 'col-md-9', parent_div = edit_panel);
}
...
function displayDivisions() {
  ...
  var edit_panel = createPanel('panel-primary', 'col-md-8', 'Family Editor', edit_row);
  displayAvailableActions(edit_panel);

  // Families Statistics
  var stats_row = createDivWithClass(null, 'row', parent_div = main);
  var stats_panel = createPanel('panel-primary', 'col-md-12', 'Number of Instances per Family', stats_row);
  var stats_panel_m = createPanel('panel-danger', 'col-md-6', 'Malicious Families', stats_panel);
  stats_panel_m.setAttribute('id', 'stats_panel_malicious');
  var stats_panel_b = createPanel('panel-success', 'col-md-6', 'Benign Families', stats_panel);
